...
st.set_page_config(
    page_title="GBS Cerebrovascular Lesion Prediction",
    page_icon="AI",
    layout="wide",
)
...
APP_TITLE = "Prediction System for Comorbid Cerebrovascular Lesions in Guillain-Barre Syndrome"
POSITIVE_CLASS = 1
...
def render_prediction(probability_map: dict[Any, float], predicted_class: Any) -> None:
    ...
    outcome_label = (
        "Predicted class: Lesion present"
        if is_positive
        else "Predicted class: Lesion absent"
    )
    chip_class = "chip-positive" if is_positive else "chip-negative"
    interpretation = (
        "The current input profile is more consistent with the presence of comorbid "
        "cerebrovascular lesions."
        if is_positive
        else "The current input profile is more consistent with the absence of comorbid "
        "cerebrovascular lesions."
    )
    ...
    with col1:
        st.markdown(
            ...
            <div class="metric-card">
                ...
                <div class="metric-label">Probability of Lesion Presence</div>
    ...
    with col2:
        st.markdown(
            f"""
            <div class="metric-card">
                <div class="metric-label">Probability of Lesion Absence</div>
                <div class="metric-value">{negative_probability:.1%}</div>
                <div class="metric-note">
                    This is the model-estimated probability for class 0.
                </div>
            </div>
            """,
            unsafe_allow_html=True,
